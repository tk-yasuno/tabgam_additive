...
対象データ:
- CMAPSS FD004: 6 operational conditions × 2 fault modes (HPC + Fan)
- 249 engines (train), 248 engines (test)
- RUL prediction (Remaining Useful Life)

問題意識:
- 航空エンジンの劣化は複数のメカニズムが同時に作用
- 複合劣化（multi-mechanism degradation）の本質を理解
...
def train_base_models(X_train, y_train, X_val, y_val):
    ...
    """
    print("=" * 80)
    print("STEP 1: Training Base Models (v0.7 - Jet Engine RUL)")
    print("=" * 80)
...
def optimize_ensemble_weights(models_dict, X_val, y_val):
    ...
    # 結果保存
    output_path = Path('outputs/results/agentic_tuning_v0.7_cmapss.json')
...
def save_results_summary(all_results, best_weights, output_path='outputs/results/all_models_comparison_v0.7_cmapss.csv'):
    """
    結果のサマリーを保存
...
def main():
    ...
    """
    print("\n" + "=" * 80)
    print("JET ENGINE DEGRADATION ANALYSIS v0.7")
    ...
    print("=" * 80)
    print("\n次のステップ:")
    print("1. 結果の可視化: python visualize_shap_cmapss_v0.7.py")
    print("2. TabNAM attention analysis for multi-mechanism degradation")
    print("3. 論文の第3ケーススタディとして執筆")
