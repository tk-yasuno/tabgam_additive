"""
論文用の可視化スクリプト（v0.6 - Material Degradation）
...
def load_results():
    """v0.6の結果を読み込み"""
    # モデル比較結果
    comparison_path = Path('outputs/results/all_models_comparison_v0.6_material.csv')
    df_comparison = pd.read_csv(comparison_path)
    df_comparison = df_comparison.set_index('Model')
    
    # 最適化履歴
    tuning_path = Path('outputs/results/agentic_tuning_v0.6_material.json')
...
def plot_model_comparison(df_comparison, save_path='outputs/figures/material_v0.6/model_comparison_v0.6.png'):
    """
    Figure 1: モデル性能比較図
    """
    ...
    ax.scatter(best_idx, df_models['rmse'].iloc[best_idx] - 0.03, 
               marker='*', s=300, color='gold', edgecolor='black', linewidth=1.5, zorder=10)
    ...
    ax.scatter(best_idx, df_models['mae'].iloc[best_idx] - 0.015, 
               marker='*', s=300, color='gold', edgecolor='black', linewidth=1.5, zorder=10)
    
    plt.suptitle('Material Degradation Analysis (v0.6): Model Performance', 
...
def plot_optimization_process(tuning_results, save_path='outputs/figures/material_v0.6/optimization_process_v0.6.png'):
    """
    ...
    plt.suptitle('Material Degradation v0.6: Agentic Weight Optimization', 
...
def plot_optimal_weights(tuning_results, save_path='outputs/figures/material_v0.6/optimal_weights_v0.6.png'):
    """
    Figure 3: 最適重みの円グラフ
    """
...
def plot_ensemble_comparison(df_comparison, save_path='outputs/figures/material_v0.6/ensemble_comparison_v0.6.png'):
    """
    Figure 4: アンサンブルと個別モデルの比較
    """
    # データ準備（R²で降順ソート）
    all_models = ['GAM', 'ENSEMBLE', 'TabNAM', 'UNIFORM_ENSEMBLE', 'GBAM', 'FAM']
    ...
    # GAM にスターマーク（最高性能）
    ax.scatter(df_all.loc['GAM', 'r2'] + 0.06, 0, 
    ...
    # 矢印で改善を示す
    ax.annotate('', xy=(ensemble_r2, 3.3), xytext=(uniform_r2, 3.3),
                arrowprops=dict(arrowstyle='<->', color='red', lw=2))
    ax.text((ensemble_r2 + uniform_r2) / 2, 3.5, 
...
def create_summary_table(df_comparison, tuning_results, save_path='outputs/figures/material_v0.6/summary_table_v0.6.png'):
    """
    ...
    table[(1, 3)].set_text_props(color='white', weight='bold')
...
def main():
    """メイン実行"""
    print("=" * 80)
    print("Generating Figures for Paper (v0.6 - Material Degradation)")
    print("=" * 80)
    ...
    print("=" * 80)
    print("\nOutput directory: outputs/figures/material_v0.6/")
    print("\nFigures:")
    print("  1. model_comparison_v0.6.png - モデル性能比較")
    print("  2. optimization_process_v0.6.png - 最適化プロセス")
    print("  3. optimal_weights_v0.6.png - 最適重み")
    print("  4. ensemble_comparison_v0.6.png - アンサンブル比較")
    print("  5. summary_table_v0.6.png - サマリーテーブル")
    print("\n論文のFigureとして使用できます。")
