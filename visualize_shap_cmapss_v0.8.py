"""
SHAP Analysis for Jet Engine Degradation (v0.8)
CMAPSS FD002: RUL Prediction with Single-Fault Degradation

This script performs SHAP analysis to interpret model predictions.
FD002 focuses on single fault mode (HPC degradation only) vs FD004's dual fault modes.
"""
...
from data_preprocessing_cmapss_v08 import load_and_preprocess_data_v08

# Output directory
os.makedirs('outputs/figures/cmapss_v0.8', exist_ok=True)
...
def plot_shap_summary(shap_values, X_sample, feature_names, output_path):
    ...
    plt.title('SHAP Feature Importance - Jet Engine RUL Prediction (FD002)', fontsize=14, weight='bold')
...
def plot_shap_bar(shap_values, feature_names, output_path):
    ...
    plt.title('SHAP Feature Importance - Mean Absolute Impact (FD002)', fontsize=14, weight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  ✅ Saved: {output_path}")
...
def plot_shap_dependence_top_features(shap_values, X_sample, feature_names, output_path, top_k=6):
    ...
    plt.suptitle('SHAP Dependence Plots - Top 6 Features (FD002 Single Fault)', fontsize=16, weight='bold', y=1.00)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  ✅ Saved: {output_path}")
...
def plot_shap_waterfall(shap_values, X_sample, feature_names, output_path, sample_idx=0):
    ...
    plt.title(f'SHAP Waterfall - Sample {sample_idx} Explanation (FD002)', fontsize=14, weight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  ✅ Saved: {output_path}")

def analyze_single_fault_impact(shap_values, X_sample, feature_names):
    """Analyze impact of different sensor groups for single-fault degradation"""
    print("\n" + "="*80)
    print("SINGLE-FAULT DEGRADATION ANALYSIS (FD002: HPC only)")
    print("="*80)
...
def main():
    print("="*80)
    print("SHAP ANALYSIS FOR JET ENGINE DEGRADATION (v0.8)")
    print("CMAPSS FD002: RUL Prediction (Single Fault Mode)")
    print("="*80)
    
    # Load and preprocess data
    print("\n--- Loading Data ---")
    data, preprocessor = load_and_preprocess_data_v08()
    ...
    plot_shap_summary(
        ...
        'outputs/figures/cmapss_v0.8/shap_summary.png'
    )
    
    plot_shap_bar(
        shap_values, feature_names,
        'outputs/figures/cmapss_v0.8/shap_bar.png'
    )
    
    plot_shap_dependence_top_features(
        shap_values, X_explain, feature_names,
        'outputs/figures/cmapss_v0.8/shap_dependence_top6.png',
        top_k=6
    )
    ...
    for i, idx in enumerate(sample_indices):
        plot_shap_waterfall(
            shap_values, X_explain, feature_names,
            f'outputs/figures/cmapss_v0.8/shap_waterfall_sample_{i+1}.png',
            sample_idx=idx
        )
    
    # Single-fault analysis
    group_importance = analyze_single_fault_impact(shap_values, X_explain, feature_names)
    ...
    output_csv = 'outputs/results/shap_feature_importance_v0.8_cmapss.csv'
    ...
    print("="*80)
    print(f"\nVisualization files saved in: outputs/figures/cmapss_v0.8/")
    print(f"Feature importance saved in: {output_csv}")
    print("\n次のステップ:")
    print("1. FD002 (1 fault) vs FD004 (2 faults) feature importance comparison")
    print("2. Identify which sensors are more critical for single-fault prediction")
