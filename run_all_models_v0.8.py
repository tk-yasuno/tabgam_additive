"""
航空エンジン劣化分析 v0.8 - CMAPSS FD002

目的:
1. Diverse Ensemble: GAM + FAM + GBAM + TabNAM の異種アンサンブル
2. 複合劣化メカニズム（摩耗 × 熱疲労 × 振動 × 運転条件）の相互作用を分析
3. アルゴリズムはv0.5と完全に同一

対象データ:
- CMAPSS FD002: 6 operational conditions × 1 fault mode (HPC degradation only)
- 260 engines (train), 259 engines (test)
- RUL prediction (Remaining Useful Life)

研究目的:
- 故障モード1つの場合（FD002）の予測精度を検証
- FD004（2故障モード）との比較分析により、故障モード複雑度の影響を理解
...
def train_base_models(X_train, y_train, X_val, y_val):
    ...
    """
    print("=" * 80)
    print("STEP 1: Training Base Models (v0.8 - Jet Engine RUL, 1 Fault Mode)")
    print("=" * 80)
...
def optimize_ensemble_weights(models_dict, X_val, y_val):
    ...
    # 結果保存
    output_path = Path('outputs/results/agentic_tuning_v0.8_cmapss.json')
...
def save_results_summary(all_results, best_weights, output_path='outputs/results/all_models_comparison_v0.8_cmapss.csv'):
    """
    結果のサマリーを保存
...
def main():
    ...
    """
    print("\n" + "=" * 80)
    print("JET ENGINE DEGRADATION ANALYSIS v0.8")
    print("Multi-Mechanism Degradation with Diverse GAMs")
    print("CMAPSS FD002: RUL Prediction (1 Fault Mode)")
    print("=" * 80)
    ...
    print(f"\nFeatures: {len(splits['feature_names'])}")
    ...
    print("=" * 80)
    print("\n次のステップ:")
    print("1. 結果の可視化: python visualize_shap_cmapss_v0.8.py")
    print("2. TabNAM attention analysis for single-fault degradation")
    print("3. FD002 (1 fault) vs FD004 (2 faults) comparison")
